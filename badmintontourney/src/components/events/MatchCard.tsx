...
const getParticipantName = (
  participant: MatchCardPropType['participant1'] | MatchCardPropType['participant2']
) => {
  if (!participant) return 'To Be Determined';

  let name = `${participant.player1.first_name} ${participant.player1.last_name ?? ''}`;
  
  if (participant.player2) {
    name += ` / ${participant.player2.first_name} ${participant.player2.last_name ?? ''}`;
  }
  return name;
};
        ...
        <Stack spacing={1.5} mt={1}>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography
              ...
              color={p1IsWinner ? 'text.primary' : 'text.secondary'}
            >
              {getParticipantName(match.participant1)}
            </Typography>
            {p1IsWinner && <CheckCircleIcon color="success" sx={{ fontSize: '1.25rem' }} />}
          </Box>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography
              ...
              color={p2IsWinner ? 'text.primary' : 'text.secondary'}
            >
              {getParticipantName(match.participant2)}
            </Typography>
            {p2IsWinner && <CheckCircleIcon color="success" sx={{ fontSize: '1.25rem' }} />}
          </Box>
        </Stack>
      </CardContent>
