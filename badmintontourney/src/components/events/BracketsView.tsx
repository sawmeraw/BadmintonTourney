import type {
    EventRoundDetailsApiResponse,
    MatchResponseParticipantProperty,
    MatchSummary,
} from "@/lib/types/api";
...
export const BracketView = ({
    ...
    data: EventRoundDetailsApiResponse;
}) => {
    return (
        <Box>
            ...
            {data.matches.length === 0 ? (
                ...
                <Grid container spacing={2}>
                    {data.matches
                        .sort(
                            (a, b) =>
                                (a.round_match_number || 0) -
                                (b.round_match_number || 0)
                        )
                        .map((match) => (
                            <Grid
                                key={match.id}
                                fontSize={{ xs: 12, sm: 6, lg: 4 }}
                            >
